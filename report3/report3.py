...
"""
expr = ['a=b+c', 'a=b+c*(d/e)', 'a=b*c+d/e', 'a=b*(c+d)/e', 'a=b+c**2^d/e&f']
"""    
        'a=b+c',
        'a = b + c * (d-e)',
        'a=b*c+d/e',
...
def makePri(str):
    ''' 演算子優先法 '''
    expr = infixNotation( (Ident^Number),
...
def makePolish(l):
    ...
    r.append( o[i]['right'] )
    while True:
        s = [x for x in r if x.startswith('_')]
        if len(s)==0: break
        j = r.index( s[0] )
        k = [x for x in o if x['no']==s[0]][0]
##       print(s, r, j, k)
        r = r[:j]+[k['val']]+[k['left']]+[k['right']]+r[j+1:]
    print(r)
    return r, reversed(o)
